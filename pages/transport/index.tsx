...
const COPY = {
  en: {
    ...
    fields: {
      ...
      kefLockerCapacityInfo:
        'Each locker (H 192 × W 41 × D 92 cm) holds about 3 suitcases — or 2 golf sets, 1 bike box, or 2–3 ski bags. {lockers} free for this slot (about {capacity} bags).',
      kefLockerOverCapacity:
        'Your {bags} bags are over the ~{capacity}-bag locker capacity for this slot. You can still book — extra lockers may be rentable on-site at The Bike Pit, subject to availability and paid at the airport.',
  ...
  is: {
    ...
    fields: {
      ...
      kefLockerCapacityInfo:
        'Hver lyklageymsla (H 192 × B 41 × D 92 cm) rúmar um 3 ferðatöskur — eða 2 golfsett, 1 hjólakassa eða 2–3 skíðatöskur. {lockers} laus(ar) fyrir þennan tíma (um það bil {capacity} töskur).',
      kefLockerOverCapacity:
        '{bags} töskurnar þínar fara yfir ~{capacity} tösku rými fyrir þennan tíma. Þú getur samt bókað — mögulega er hægt að leigja fleiri geymslur á staðnum hjá The Bike Pit, eftir framboði og greitt á flugvellinum.',
...
const TransportPage = () => {
  ...
  // Lockers free for the customer's pickup slot (KEF locker mode). Each locker
  // fits ~3 suitcases, so capacity ≈ lockers × 3. Drives the advisory
  // over-capacity notice — warning only, the booking is still allowed (they can
  // rent more on-site at the airport).
  const KEF_SUITCASES_PER_LOCKER = 3
  const kefLockersAvailable = (() => {
    const data = kefPickupAvailability.data
    if (!data) return null
    const m = /^(\d{1,2}):(\d{2})/.exec(booking.pickupTime || '')
    const landingHour = m ? parseInt(m[1], 10) + parseInt(m[2], 10) / 60 : 0
    if (data.shifts && data.shifts.length > 0) {
      const shift = data.shifts.find((s) => landingHour < s.pickupHourDecimal)
      return shift ? shift.available : 0
    }
    return Math.max(0, data.lockerCapacity - data.lockersInUse)
  })()
  const kefLockerCapacityBags =
    kefLockersAvailable != null ? kefLockersAvailable * KEF_SUITCASES_PER_LOCKER : null
  const kefOverCapacity =
    kefPickupMode === 'locker' &&
    kefLockerCapacityBags != null &&
    Number(booking.bagsCount) > kefLockerCapacityBags
      ...
      <>
        {meta.requiresFlightNumber && (
          <Field>
            ...
            {/* Locker capacity guidance + over-capacity advisory (KEF locker
                pickup only). Each locker ~3 suitcases; warning is advisory —
                booking is still allowed. */}
            {meta.kind === 'kef-airport' && isPickup && kefPickupMode === 'locker' && (
              <>
                <BsiInfoNote>
                  {t.fields.kefLockerCapacityInfo
                    .replace('{lockers}', String(kefLockersAvailable ?? 2))
                    .replace('{capacity}', String(kefLockerCapacityBags ?? 6))}
                </BsiInfoNote>
                {kefOverCapacity && (
                  <BsiLockerNote>
                    {t.fields.kefLockerOverCapacity
                      .replace('{bags}', String(booking.bagsCount))
                      .replace('{capacity}', String(kefLockerCapacityBags ?? 6))}
                  </BsiLockerNote>
                )}
              </>
