...
const DatePickerGlobal = () => (
  <Global
    styles={css`
      .bagbee-datepicker-wrapper {
        width: 100%;
      }
      .bagbee-datepicker-wrapper .react-datepicker__input-container input {
        ${BaseInputStyles}
        cursor: pointer;
        width: 100%;
      }
      ...
      .react-datepicker {
        ...
        border: 1.5px solid #e0d7c7;
        border-radius: 16px;
        box-shadow: 0 12px 40px -12px rgba(29, 60, 52, 0.2);
        overflow: hidden;
      }
...
const CHECKOUT_TIMES_LOCKER = timeOptions(17, 15, 23)
        ...
        {submitted ? (
          ...
          <>
                      ...
                      <Controller
                        ...
                        render={({ field }) => (
                          <DatePicker
                            ...
                            wrapperClassName='bagbee-datepicker-wrapper'
                            autoComplete='off'
                          />
                        )}
                      />
                        ...
                        render={({ field }) => (
                          <Select
                            ...
                            onChange={(e) => {
                              ...
                              const isLocker = CHECKOUT_TIMES_LOCKER.includes(e.target.value)
                              setValue('late', isLocker)
                            }}
                          >
                            <option value=''>Select time</option>
                            <optgroup label='Staffed pickup (before 5 PM)'>
                              {CHECKOUT_TIMES_NORMAL.map((t) => (
                                <option key={t} value={t}>{t}</option>
                              ))}
                            </optgroup>
                            <optgroup label='Locker delivery (after 5 PM — +500 ISK/bag)'>
                              {CHECKOUT_TIMES_LOCKER.map((t) => (
                                <option key={t} value={t}>{t}</option>
                              ))}
                            </optgroup>
                          </Select>
            ...
            <Summary>
              ...
              <BulletList>
                <li>Pay securely online by card — powered by Rapyd.</li>
