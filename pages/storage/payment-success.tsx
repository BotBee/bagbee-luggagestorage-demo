...
const Icon = styled.div<{ error?: boolean }>`
  ...
  height: 72px;
  border-radius: 50%;
  background: ${({ error }) => (error ? '#fee2e2' : '#dcfce7')};
  display: flex;
  align-items: center;
  justify-content: center;
  font-size: 32px;
...
export default function StoragePaymentSuccess() {
  ...
  return (
    <Page>
      ...
      <Card>
        <Icon>{loading ? '⏳' : '✅'}</Icon>
        <Title>{loading ? 'Confirming payment…' : 'Booking confirmed!'}</Title>
        {!loading && (
          <>
            <Body>
              Your luggage storage is booked and paid. We&apos;ll see you at BSÍ!{' '}
              {bookingId && (
                <>
                  You can view or change your booking at any time via your booking link.
                </>
              )}
            </Body>
            <div>
              {bookingId && (
                <Btn href={`/storage/${bookingId}`}>View booking</Btn>
              )}
