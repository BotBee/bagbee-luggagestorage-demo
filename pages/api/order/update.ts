...
export default async function handler(
  ...
  try {
    ...
    //
    // The 750ms wait between the clear and the set is intentional. Without it,
    // Airtable coalesces the two PATCHes into a single "no net change" event
    // when the previous state of the field was already ['Update OC'] — the
    // automation engine never sees the field transition out of and back into
    // the matched-condition view, so the "send order confirmation" automation
    // doesn't fire on second-and-subsequent updates by the same customer. The
    // sleep gives Airtable's change-detection layer enough time to flush the
    // clear before the set arrives, so it sees them as two distinct events.
