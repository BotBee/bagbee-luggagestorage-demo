...
export default async function handler(
  ...
  try {
    ...
    // requests, otherwise anyone could write arbitrary tip amounts to any
    // order without paying. See utils/paymentRedirectSig.ts.
    const signedParams = {
      orderNo,
      amount: String(Math.round(tipAmount)),
      recordId: record.id,
    }
    const { exp, sig } = signRedirectParams('tip', TIP_SIGNED_FIELDS, signedParams)
    const params = new URLSearchParams({ ...signedParams, exp, sig })
