...
/**
 * Validates a Fast-Track discount code (exists + not expired) against the
 * Afslattarkodar Fast Track table. Lookup + injection guard live in
...
export default async function handler(
  ...
  try {
    res
      .status(200)
      .json(await lookupDiscountCode('Afslattarkodar Fast Track', code))
  } catch (error) {
