  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.log('[api][air-travel][get-flights] error', errorResponse) // Log the error response
      ...
      console.log('[api][air-travel][get-flights] error', error)
      ...
      return res.status(500).json({ error: 'An unknown error occurred while fetching data' })
    }
  } else {
