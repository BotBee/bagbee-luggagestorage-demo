...
const sanitizeChanges = (
  ...
  for (const key of EDITABLE_KEYS) {
    ...
    if (key === 'pax') {
      // Pax can be cleared back to null OR set to a positive integer.
      // Empty string / 0 normalises to null so the calculator falls
      // back to the bag-conversion estimate.
      if (v == null || v === '' || v === 0) {
        out[key] = null
      } else {
        const n = typeof v === 'number' ? v : Number(v)
    ...
    try {
      ...
      if (before) {
        ...
        for (const key of Object.keys(changes) as EditableField[]) {
          let bv = (b[key] ?? null) as string | number | null
          let av = (a[key] ?? null) as string | number | null
          // For the Notes / comment field, strip any legacy auto-appended
          // "— Partner edit by … @ … UTC:" blocks before comparing. The
          // auto-append was removed in 2026-06, but orders that were
          // bloated before then still carry the changelog text in their
          // comment field — without this strip the email Before/After
          // would show the same multi-page wall of text we removed.
          // Net effect for fresh orders: no-op (no markers to strip).
          // Net effect for legacy bloated orders: the email shows only
          // the partner's actual typed note, not the historical cruft.
          if (key === 'comment') {
            bv = stripPartnerChangelog(typeof bv === 'string' ? bv : '')
            av = stripPartnerChangelog(typeof av === 'string' ? av : '')
          }
