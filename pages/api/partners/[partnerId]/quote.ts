...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // Pax is optional. Empty / non-numeric falls back to null so
  // computeOrderPrice's bag→pax estimate kicks in.
  const paxNum = typeof pax === 'string' ? Number(pax) : NaN
  const input = {
    ...
    pax: Number.isFinite(paxNum) && paxNum > 0 ? Math.floor(paxNum) : null,
    timeWindow: typeof timeWindow === 'string' ? timeWindow : null,
    deliveryTimeWindow:
      typeof deliveryTimeWindow === 'string' ? deliveryTimeWindow : null,
