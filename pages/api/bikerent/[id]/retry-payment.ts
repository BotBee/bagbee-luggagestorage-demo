...
/**
 * Re-open a Rapyd checkout for an existing Pending BikeRent booking so the
 * customer can retry a failed/cancelled payment without re-entering anything.
 *
 * POST /api/bikerent/[id]/retry-payment  →  { redirectUrl }
 */
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    console.error('[BikeRent retry] Failed:', err)
