...
/**
 * Customer-initiated cancel + auto-refund for a BikeRent booking. Mirrors the
 * storage cancel flow. `refundStorageBooking` is field-name generic — it only
 * touches `Payment Status` + `Rapyd Payment ID`, which the BikeRent table now
 * has — so it's reused as-is.
 *
 *   Cancelled / Refunded → idempotent 200
 *   Paid                 → Rapyd refund, then Refunded (+ Cancelled checkbox)
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (currentStatus === 'Cancelled') return res.status(200).json({ status: 'Cancelled' })
    if (currentStatus === 'Refunded') return res.status(200).json({ status: 'Refunded' })
    ...
    if (currentStatus === 'Paid') {
      ...
      if (result.status === 'refunded') {
        // refundStorageBooking already set Payment Status = 'Refunded' + unticked Paid?.
      ...
      if (result.status === 'no_payment') {
        console.warn('[BikeRent cancel] Paid booking has no Rapyd Payment ID — manual refund required:', id)
        await table.update(id, { 'Payment Status': 'Cancelled', 'Paid?': false } as FieldSet)
        await notifyBookingEvent(id, 'cancelled', false)
    ...
    console.error('[BikeRent cancel] Failed:', err)
