...
import type { Error as AirtableError, FieldSet, Record } from 'airtable'
  ...
  const onSubmit = async (values: Customer) => {
    ...
    try {
      ...
      if (order && order.id) {
        ...
        // createFastTrackOrder fetches with res.json() regardless of status,
        // so a 400/500 surfaces here as { message } instead of a record.
        // Without throwing, react-hook-form treats the submit as successful
        // → isSubmitSuccessful=true → the button wedges in a permanent
        // spinner with no visible error (same bug fixed on
        // /book/confirm-order 2026-05-04).
        const serverMessage = (order as any)?.message || (order as any)?.error
        toast.error(
          serverMessage ||
            (locale === 'en'
        ...
        console.error(
          '[fast-track/confirm-order] createFastTrackOrder returned no record id',
          order,
        )
        throw new Error(serverMessage || 'createFastTrackOrder failed')
      }
    } catch (error) {
      // TODO: Add sentry
      console.error(error)
      // Re-throw so react-hook-form sees the failure and isSubmitSuccessful
      // stays false — otherwise the button's loading state (bound to
      // isSubmitting || isSubmitSuccessful) never clears and the customer
      // can't retry without reloading the page.
      throw error
    }
  }
