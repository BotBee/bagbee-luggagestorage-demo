...
// Yellow-ish "you're in duplicate mode" callout shown above the form
// when the URL has ?duplicate=. The colour mirrors the partner-side
// tentative-pricing disclaimer style so the partner instantly reads
// this as "informational, not an error".
...
type Form = {
  ...
  flightNumber: string
  // Pax stored as a string so the form can render an empty input — empty
  // means "not entered", which the calculator interprets as "fall back
  // to ceil(bags / 1.5)". Iceland Travel bills by pax on BSI to Hotel +
  // Capital area transfer tiers, so entering pax here gives them an
  // accurate quote without relying on the bag/pax ratio heuristic.
  pax: string
  bagsRegular: string
  bagsOdd: string
...
export default function NewPartnerOrder({ partnerId, partnerDisplayName, sessionEmail }: Props) {
  ...
  // Duplicate-mode metadata: when the URL contains ?duplicate=<recId>
  // we fetch that order and pre-fill the form below. duplicateFrom
  // captures the source order's short ref so the banner can say
  // "Duplicating from ATL013385" and the PM knows the form isn't blank
  // by accident.
  ...
  useEffect(() => {
    ...
    const paxNum = Number(form.pax) || 0
    // Quote needs at least one of (bags, pax) — the calculator derives
    // the missing dimension via the 1.5-bags-per-pax heuristic.
    if (!form.serviceType || (totalBags <= 0 && paxNum <= 0)) {
      setQuote(null)
    ...
    const t = setTimeout(async () => {
      try {
        const params = new URLSearchParams({
          ...
          // Only send pax to the quote endpoint when the field has a
          // positive integer — empty / 0 / NaN are dropped so the server
          // falls back to its bag→pax estimate.
          ...(Number(form.pax) > 0
            ? { pax: String(Math.floor(Number(form.pax))) }
            : {}),
          timeWindow: form.timeWindow,
          // Delivery time also drives the time-of-day surcharge — a
  ...
  // Duplicate flow: when the URL has ?duplicate=<recordId>, fetch that
  // order from the partner-scoped GET endpoint and copy ALL its fields
  // into the form state. The PM amends whatever needs to change (most
  // commonly the date and reference) and submits as a new order.
  //
  // We don't clear date/ref/notes on purpose — keeping them visible
  // makes it obvious WHICH fields the source had set, so the PM can
  // see what to change. The submit handler creates a brand new record
  // either way, so there's no risk of accidentally editing the source.
  //
  // Errors here are non-fatal: if the fetch fails (deleted record,
  // network blip, scope mismatch) we just leave the form empty with a
  // toast so the PM can fall back to manual entry.
  useEffect(() => {
    if (!router.isReady) return
    const dup = router.query.duplicate
    ...
    fetch(`/api/partners/${partnerId}/orders/${dup}`, { credentials: 'same-origin' })
      ...
      .then((data) => {
        ...
        const n = (k: string) =>
          typeof o[k] === 'number' ? String(o[k] as number) : s(k)
        setForm((f) => ({
          ...f,
          customerName: s('customerName') || f.customerName,
          contactName: s('contactName') || f.contactName,
          reference: s('reference'),
          email: s('email') || f.email,
          phone: s('phone'),
          serviceType: s('serviceType') || 'Pickup & Delivery',
          ...
          deliveryTimeWindow: s('deliveryTimeWindow'),
          flightNumber: s('flightNumber'),
          pax: n('pax'),
          bagsRegular: n('bagsRegular'),
          bagsOdd: n('bagsOdd') || '0',
          // Don't carry the source price — the live quote will recompute
          // from the duplicated fields. Carrying it would mislead the
          // PM into thinking the price is locked.
          estimatedAmount: '',
          comment: s('comment'),
        }))
        setDuplicateFrom(s('orderNoShort') || s('reference') || dup.slice(-5))
      })
      .catch(() => {
        if (!cancelled) {
          setToast({
            kind: 'err',
            msg: 'Could not load the source order — start from blank.',
          })
        }
      })
      .finally(() => {
  ...
  }, [router.isReady, router.query.duplicate, partnerId])
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        // Only send pax when the partner typed a positive value. Empty
        // = "no pax specified, use bag→pax estimate downstream".
        pax: Number(form.pax) > 0 ? Math.floor(Number(form.pax)) : undefined,
  ...
  return (
    <PartnerLayout partnerId={partnerId} partnerDisplayName={partnerDisplayName}>
      ...
      <Title>{duplicateFrom ? 'Duplicate booking' : 'New booking'}</Title>
      <Sub>
      ...
      {duplicateFrom && (
        <DuplicateBanner>
          {duplicateLoading
            ? `Loading source order ${duplicateFrom}…`
            : `Pre-filled from ${duplicateFrom}. Amend the date, reference, and notes — submit creates a brand-new booking.`}
        </DuplicateBanner>
      )}
