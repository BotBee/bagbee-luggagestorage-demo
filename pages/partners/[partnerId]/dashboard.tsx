...
import { stafferNameFromEmail } from '../../../utils/partnerStafferName'
...
const Row = styled.tr`
  ...
  @media (max-width: ${MOBILE_BREAKPOINT}) {
    ...
    & > td[data-label='Origin'] { grid-area: from; }
    & > td[data-label='Destination'] { grid-area: to; }
    & > td[data-label='Total bags'] { grid-area: bags; justify-self: end; }
    & > td[data-label='Project manager'] { grid-area: driver; }
...
const fmtDateWeekdayLong = (s: string | null) => {
  if (!s) return '—'
  const d = new Date(s)
  if (Number.isNaN(d.getTime())) return s
...
export default function PartnerDashboard({
  ...
  const visible = useMemo(() => {
    ...
    return orders.filter((o) => {
      ...
      const hay = [
        o.driverName,
        // Project-manager name + email so typing "ragna" or
        // "atlantik.is" matches the visible PM column.
        o.email,
        stafferNameFromEmail(o.email),
        // Include BOTH the stored service value AND its display label so
        // typing "luggage transfer" finds Pickup-&-Delivery orders.
        o.serviceType,
                ...
                return (
                  ...
                  >
                    ...
                    <Td data-label="Pickup">
                      ...
                      {/* Time window already shows under the From/To
                          location cells — no need to repeat it here.
                          Spell the weekday in full so the column reads
                          professionally at a glance ("Monday" rather
                          than "Mon"). */}
                      <div style={{ color: '#696f79', fontSize: 11, marginTop: 2 }}>
                        {fmtDateWeekdayLong(o.pickupDate)}
                      </div>
                    </Td>
                    <Td data-label="Service">{serviceTypeLabel(o.serviceType) || '—'}</Td>
                    ...
                    <Td data-label="Project manager">
                      {(() => {
                        // Project manager = the partner staffer who created
                        // the order, derived from the email field on the
                        // record. Replaces the previous "Driver" column —
                        // partners care about who on THEIR side owns the
                        // booking, not which BagBee driver is on the route.
                        const pmName = stafferNameFromEmail(o.email)
                        if (!pmName) {
                          return <Unassigned>Unassigned</Unassigned>
                        }
                        return (
                          <Driver onClick={(e) => e.stopPropagation()}>
                            <DriverName>{pmName}</DriverName>
                            {o.email ? (
                              <PhoneLink href={`mailto:${o.email}`}>
                                {o.email}
                              </PhoneLink>
                            ) : null}
                          </Driver>
