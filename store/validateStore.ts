...
/** Strips query/hash and optional /en or /is locale prefix so validation matches ApplicationRoutes paths. */
export function normalizePathForValidateStore(location: string): string {
  let path = location.split('?')[0].split('#')[0]
  if (path.startsWith('/en/')) {
    path = `/${path.slice(4)}`
  } else if (path === '/en') {
    path = '/'
  } else if (path.startsWith('/is/')) {
    path = `/${path.slice(4)}`
  } else if (path === '/is') {
    path = '/'
  }
...
export const validateStore = (location: string, booking: Booking): boolean => {
  const locationPath = normalizePathForValidateStore(location)
