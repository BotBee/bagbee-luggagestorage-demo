...
const AvailablePickupTimes = () => {
  ...
  // Pickup address drives the cruise-harbour capacity exemption server-side.
  // Part of the query key so picking the pier re-evaluates slot availability.
  const pickupAddress = bookingState.pickupInformation.pickupLocation ?? ''
  ...
  const { data, isPending } = useQuery({
    queryKey: ['available-pickup-times', scheduledDeparture, postalCode, pickupAddress],
    enabled: Boolean(scheduledDeparture),
    queryFn: () =>
      fetch('/api/airtable/availability', {
        method: 'POST',
        mode: 'cors',
        body: JSON.stringify({ departureDate: scheduledDeparture, postalCode, pickupAddress }),
      })
        .then((res) => res.json())
        .catch(() => {
