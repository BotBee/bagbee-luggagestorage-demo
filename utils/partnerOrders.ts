...
// Pull the first Starfsmenn record ID out of a Driver override field
// value. The schema is multipleRecordLinks (Airtable API can't enforce
// prefersSingleRecordLink post-creation), so the raw value comes back
// as either an array of record IDs or — when the linked record is the
// primary field expansion — an array of `{id, name}` objects. We
// tolerate both shapes the same way resolveDriver does.
const firstOverrideId = (raw: unknown): string | null => {
  if (!Array.isArray(raw) || raw.length === 0) return null
  const first = raw[0]
...
const attachDriverInfo = async (
  ...
  overrideByOrderId: Map<string, unknown>,
): Promise<void> => {
  const staff = await loadStaff()
  for (const o of orders) {
    // Per-order Driver override beats the shift-level Bílstjóri lookup —
    // set on individual orders that need to be reassigned away from the
    // shift's default driver (the common case: a 2-driver evening
    // shift where Runar takes some orders and Matas takes others).
    // Empty override → fall through to the existing shift lookup so
    // single-driver shifts stay zero-effort.
    const overrideId = firstOverrideId(overrideByOrderId.get(o.id))
    if (overrideId) {
      const staffInfo = staff.byId.get(overrideId)
      if (staffInfo) {
        o.driverRecordId = overrideId
        o.driverName = staffInfo.name
        o.driverPhone = staffInfo.phone
        continue
      }
    }
...
export const FIELDS = {
  ...
  // Number of passengers (Atlantik + Iceland Travel both bill pax-based
  // tiers — BSI to Hotel, Capital area transfer — on the actual pax
  // count, not on ceil(bags / 1.5). When this field is populated it
  // takes precedence over the bag→pax conversion in computeOrderPrice.
  pax: 'fldqA0qw4Chjn6lew', // "PAX" (number, writeable)
  ...
  // Per-order driver override (linked to Starfsmenn, single-link convention
  // even though the schema is multipleRecordLinks — Airtable's API can't
  // toggle prefersSingleRecordLink, so the constraint is enforced by
  // convention + UI toggle). Set ONLY on orders that need to be
  // re-assigned away from the shift's default driver — typically on
  // 2-driver evenings where Runar takes some orders and Matas takes
  // others. Empty = inherit the shift's driver(s) from `driver` above.
  driverOverride: 'fldrRFBuDWAtX5cfS',
...
export type OrderSummary = {
  ...
  bagsRegular: number
  bagsOdd: number
  // Number of passengers (PAX column). Drives pax-based pricing tiers
  // (BSI to Hotel, Capital area transfer) when populated. null = use
  // ceil(totalBags / 1.5) fallback in computeOrderPrice.
  pax: number | null
  amount: number
...
const mapRecord = (record: AirtableRecord<FieldSet>): OrderSummary => {
  ...
  return {
    ...
    // PAX is null (not 0) when unset so the pricing calculator can
    // distinguish "no pax specified, fall back to bag-conversion" from
    // "explicit 0 pax" (which would be a weird booking but still
    // legitimately user-entered).
    pax: typeof get(F_NAMES.pax) === 'number' ? (get(F_NAMES.pax) as number) : null,
...
export const listPartnerOrders = async (partnerId: PartnerId): Promise<OrderSummary[]> => {
  ...
  const rawOverrides = new Map<string, unknown>()
  for (const r of records) {
    rawDrivers.set(r.id, (r.fields as Record<string, unknown>)[F_NAMES.driver])
    rawOverrides.set(r.id, (r.fields as Record<string, unknown>)[F_NAMES.driverOverride])
  }
  await attachDriverInfo(summaries, rawDrivers, rawOverrides)
  return summaries
}
...
export const getPartnerOrder = async (
  ...
  const rawOverride = new Map<string, unknown>([
    [record.id, (record.fields as Record<string, unknown>)[F_NAMES.driverOverride]],
  ])
  await attachDriverInfo([summary], raw, rawOverride)
  return summary
}

// Strip any auto-appended "— Partner edit by … @ … UTC:" blocks that
// older builds added to the Notes / comment field. Returns the
// partner-typed portion that precedes the first changelog marker. Used:
//   - by the email-diff endpoint, so the bagbee@ "Notes" Before/After
//     shows only the partner's actual typed text instead of the full
//     bloated history;
//   - by the edit form's pre-fill, so the partner sees their original
//     notes (not a wall of changelog cruft) when they reopen an order.
//
// New orders never accumulate this anymore — the buildChangelog +
// append-to-comment behaviour was removed in 2026-06. This function
// exists purely to clean up records that were already bloated when the
// behaviour shipped.
export const stripPartnerChangelog = (text: string | null | undefined): string => {
  if (!text) return ''
  // Match one changelog block: the "— Partner edit by NAME @ TS UTC:"
  // header plus everything up to the next changelog header or end of
  // string. The lazy [\s\S]*? handles bullet lines with arbitrary
  // content, and the lookahead stops cleanly at the next block boundary
  // so we don't accidentally swallow partner-typed text that happens
  // to be sandwiched between two changelog blocks.
  const blockRe =
    /\n?— Partner edit by [^\n]+ @ \d{4}-\d{2}-\d{2} \d{2}:\d{2} UTC:[\s\S]*?(?=\n— Partner edit by |$)/g
  return text.replace(blockRe, '').replace(/\s+$/, '')
}
...
export const updatePartnerOrder = async (
  ...
  // NOTE: previously we appended a "— Partner edit by … @ … UTC: • field:
  // from → to" line to the Notes / comment field on every edit, intended
  // as an in-Airtable audit trail. In practice the field grew unbounded
  // (every edit captured the bloated previous value of the comment as
  // its own "from", quadratically) and the diff in the bagbee@ "Notes"
  // Before/After email became multi-page unreadable. Audit history now
  // lives only in the email stream + Airtable's record revision history;
  // the comment field stays pure partner-typed text. The `actor`
  // parameter is kept on the signature for future use (and to avoid
  // churning the two callers in pages/api/.../[recordId]).

  // Suppress the customer-facing "your order was updated" email when the
  // order hasn't been marked paid yet. PMs commonly tweak a brand-new
  // booking a few times before the customer pays — and firing the
  // "Update OC" trigger on each tweak makes the customer (and PM)
  // receive an "order updated" email BEFORE the booking-confirmation
  // email has even gone out. Status 'Pending' is the pre-payment state;
  // once the order is marked paid it flips to 'Confirmed' (or later),
  // and from that point on edits do fire the update email as before.
  //
  // The changelog still gets appended to the comment field above, so
  // ops can see the pre-payment edits in Airtable. The internal
  // sendOrderUpdateNotice (going to bagbee@bagbee.is) also still fires
  // from the calling PATCH endpoint — the team sees every edit
  // regardless of paid status. Only the customer-facing email is gated.
  const isPaid = before.status != null && before.status !== 'Pending'
  // Two-step update: clear the trigger so re-saves still fire when the
  // order IS paid, then set it (or leave it cleared when still Pending).
  // The 750ms wait between clear and set is REQUIRED — without it Airtable
  // coalesces the two PATCHes when the field was already ['Update OC'],
  // and the customer-facing automation never sees the transition. See
  // /api/order/update.ts for the same fix on the customer-facing path.
  ...
  await table.update(recordId, {
    ...fields,
    ...(isPaid ? { [FIELDS.updateTrigger]: ['Update OC'] } : {}),
  })
...
export type NewOrderInput = {
  ...
  bagsRegular: number
  bagsOdd: number
  // Pax (passenger count). Optional. When set, the pricing calculator
  // uses it on pax-based tiers (BSI to Hotel, Capital area transfer)
  // instead of the ceil(bags / 1.5) estimate — partners are billed
  // by pax, so this is the right denominator for an accurate quote.
  pax?: number
...
export const createPartnerOrder = async (
  ...
  const fields: Record<string, unknown> = {
    ...
    // Pax is optional on creation. Only write when set, so the
    // Airtable PAX column stays null (not 0) for orders without an
    // explicit pax count — the pricing calculator distinguishes "null
    // → use bag-conversion estimate" from "explicit number → use it".
    ...(typeof input.pax === 'number' && input.pax > 0
      ? { [FIELDS.pax]: Math.floor(input.pax) }
      : {}),
