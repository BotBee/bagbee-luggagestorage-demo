...
export const FLD = {
  ...
  bookings: {
    ...
    // Reused existing fields + the paid bike-box form fields (added 2026-06-04).
    email: 'fld3G56YeW7KcqW28',
    phone: 'fld6PFymBjzCdJtoC',
    paid: 'fldw7Mvg9HSAVLDdY',
    amount: 'fld5fl3BUfGyyhXUU',
    typeOfService: 'fldZB3z9EtBOhFAFT',
    boxes: 'fld9fcTi6hx6mUthz',
    arrivalFlight: 'fldarCei2VSdpOb1T',
    customerComment: 'fldH4UDWas7PSQMWQ',
    // Exact-window bounds (window START = the check-in/out datetime fields above):
    checkInWindowEnd: 'fldqYO7tfkvCttOYE',
    checkOutWindowEnd: 'fld52XDhbIc8d7NcL',
    // Paid-flow lifecycle:
    paymentStatus: 'fldGDzf6pkgDjhygQ',
    rapydCheckoutId: 'fldIroAIQuB4sq3ym',
    rapydPaymentId: 'fldst18bbw5KySUlI',
    returnLocation: 'fldTvyYwwVayWHV4j',
    departureFlight: 'fldYDTiCJPoRX26gS',
    // Phase 2 handoff monitor idempotency flags:
    handoffOutgoingAlerted: 'fldmBeasV9pfyesTn',
    handoffIncomingAlerted: 'fldW7yZ3zIoZ6ZTHu',
    // Plain-text locker names (formula) for emails/logs:
    lockerNameIn: 'fldxFjye7FMvXpQAX',
    lockerNameOut: 'fldkNhS49DOqyjR9I',
    // Paid booking-confirmation email (with manage link) idempotency flag:
    confirmationSent: 'fldnf5X5foCySjTgW',
  },
  // Lockers
  lockers: {
...
export async function loadLockerShiftRules(): Promise<LockerShiftRule[]> {
  ...
  const base = getBase()
  // filterByFormula must use field NAMES (not IDs) — Airtable's formula
  // parser doesn't accept field IDs. The read path uses `record.get(fieldId)`
  // so we also need returnFieldsByFieldId: true or every get() returns undefined
  // (same gotcha as loadActiveBookings above).
